...
{
    std::clog << "Git-Crypt, written by Andrew Ayer, ported to Windows by Cyril Cleaud\n";
    std::clog << "\n";
	...
	std::clog << "Plumbing commands (called by git according to .gitattributes):\n";
...
int main (int argc, const char** argv)
{
    try {
        ...
        if (_setmode(_fileno(stdin), _O_BINARY ) == -1)
            throw std::ios_base::failure("failed to switch stdin to binary mode.");
        if ( _setmode(_fileno(stdout), _O_BINARY ) == -1)
            throw std::ios_base::failure("failed to switch stdout to binary mode.");
#endif

...
void diff (const char* keyfile, const char* filename)
{
	...
	if (!in) {
	    std::clog << filename << ": problem loading file (in " << __FUNCTION__ << ")." << std::endl;
		perror(filename);
		std::exit(1);
	}
...
void init (const char* argv0, const char* keyfile)
{
	if (access(keyfile, R_OK) == -1) {
        std::clog << keyfile << ": problem accessing file (in " << __FUNCTION__ << ")." << std::endl;
		perror(keyfile);
		std::exit(1);
	}
...
#if defined(__WIN32__)
	bool			head_exists = system("git rev-parse HEAD >nul 2>nul") == 0;
#else
	...
	// For Windows only, replace '\' with '/' in order to avoid trouble when git invoke the filter.
	// This is because git eats the '\' and give a full file name like "c:keyfile" instead of "c:\keyfile".
	std::replace( keyfile_path.begin(), keyfile_path.end(), '\\', '/');
#else
	std::string	git_crypt_path(std::strchr(argv0, '/') ? resolve_path(argv0) : argv0);
#endif
	...
	command = "git config filter.git-crypt.smudge ";
	command += escape_shell_arg(escape_shell_arg(git_crypt_path) + " smudge " + escape_shell_arg(keyfile_path));
	if (system(command.c_str()) != 0) {
		std::clog << "git config failed for filter.git-crypt.smudge (in " << __FUNCTION__ << ")." << std::endl;
		std::exit(1);
	}
	...
	if (system(command.c_str()) != 0) {
		std::clog << "git config failed for filter.git-crypt.clean (in " << __FUNCTION__ << ")." << std::endl;
		std::exit(1);
	}

	// git config --bool filter.git-crypt.required true
	command = "git config --bool filter.git-crypt.required true";
	if (system(command.c_str()) != 0) {
		std::clog << "git config failed for filter.git-crypt.required (in " << __FUNCTION__ << ")." << std::endl;
		std::exit(1);
	}
	...
	if (system(command.c_str()) != 0) {
		std::clog << "git config failed for diff.git-crypt.textconv (in " << __FUNCTION__ << ")." << std::endl;
		std::exit(1);
	}

	// git config --bool diff.git-crypt.required true
	command = "git config --bool diff.git-crypt.required true";
	if (system(command.c_str()) != 0) {
		std::clog << "git config failed for diff.git-crypt.required (in " << __FUNCTION__ << ")." << std::endl;
		std::exit(1);
	}
	...
	if (head_exists) {
		...
		if (system(command.c_str()) != 0) {
            std::clog << "git checkout failed (in " << __FUNCTION__ << ")." << std::endl;
			std::clog << "git-crypt has been set up but existing encrypted files have not been decrypted\n";
			std::exit(1);
		}
	}

	// 5. Tell the user to create a .gitattributes files
	std::clog << "You should now create / update your .gitattributes file at root of therepository." << std::endl;
	std::clog << "The file may contain something like this:" << std::endl;
	std::clog << "  *.cpp filter=git-crypt diff=git-crypt" << std::endl;
	std::clog << "  yourSecretFle.dat filter=git-crypt diff=git-crypt" << std::endl;
}
...
void keygen (const char* keyfile)
{
	...
	if (!keyout) {
	    std::clog << keyout << ": problem writing file (in " << __FUNCTION__ << ")." << std::endl;
		perror(keyfile);
		std::exit(1);
	}
	umask(old_umask);
